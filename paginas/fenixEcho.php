<!-- Menu Principal-->
<div class="menuprincipal my-2 w-100 rounded">
    <div class="container">
        <div class="row">

            <!-- Coluna Logomarca -->
            <div class="col-2  logomarca">
                <img class="" src="http://placehold.it/70x100" alt="">
            </div>
            <!-- Fim Coluna Logomarca -->

            <!-- Coluna Partes/Paginas -->
            <div class="col-6">
                <div class="row">
                    <div class="col letrasmenu mt-5">
                        <h5>Serviços</h5>
                    </div>
                    <div class="col letrasmenu mt-5">
                        <h5>Portifólio</h5>
                    </div>
                    <div class="col letrasmenu mt-5">
                        <h5>Sobre</h5>
                    </div>
                    <div class="col letrasmenu mt-5">
                        <h5>Contato</h5>
                    </div>
                </div>
            </div>
            <!-- Fim Coluna Partes/Paginas -->

            <!-- Coluna Rede Sociais -->

            <!-- Fim Coluna Redes Sociais -->

        </div>
    </div>
</div>
<!-- Fim Menu Principal -->

<!-- Corpo Site -->
<div class="container">
    <img src="img/imgSite2.png" alt="" class="my-3">

    <!-- Linha Serviços -->

    <!-- Menu Serviços (Bootstrap) -->
    <div class="row">
        <div class="col-6">
            <div class="row border border-light rounded p-3">
                <div class="col-6">
                    <h5 class="text-light">WebSites</h5>

                    <div class="list-group w-75" id="list-tab" role="tablist">
                        <a class="list-group-item list-group-item-action active list-group-item-danger letramaisescura mt-3" id="list-home-list" data-bs-toggle="list" href="#list-home" role="tab" aria-controls="list-home">Institucional</a>
                        <a class="list-group-item list-group-item-action list-group-item-danger letramaisescura" id="list-profile-list" data-bs-toggle="list" href="#list-profile" role="tab" aria-controls="list-profile">Página de Vendas</a>
                        <a class="list-group-item list-group-item-action list-group-item-danger letramaisescura" id="list-messages-list" data-bs-toggle="list" href="#list-messages" role="tab" aria-controls="list-messages">Blog Pessoal</a>
                        <a class="list-group-item list-group-item-action  list-group-item-danger letramaisescura" id="list-settings-list" data-bs-toggle="list" href="#list-settings" role="tab" aria-controls="list-settings">Loja Virtual</a>
                    </div>
                </div>
                <div class="col-6 mt-3">
                    <div class="tab-content mt-5 text-light text-center" id="nav-tabContent">
                        <div class="tab-pane fade show active" id="list-home" role="tabpanel" aria-labelledby="list-home-list">
                            Site com intuito de apresentar a Empresa, a Equipe, o Portifólio, os Serviços e os Parceiros. <br>
                            <Strong class="letramaisclara text-center"> "É a sua casa na Internet"</Strong>
                        </div>
                        <div class="tab-pane fade" id="list-profile" role="tabpanel" aria-labelledby="list-profile-list">
                            Site de página única com o intuito de promover. Seja um Evento, Lançamento, Vendas Rápidas de Serviços
                            ou Prodtos.</div>
                        <div class="tab-pane fade" id="list-messages" role="tabpanel" aria-labelledby="list-messages-list">Pessoal ou Profissional,
                            o blog é um site com o intuito de se comunicar com o mundo. Seja para escrever sobre suas opniões, sua jornada,
                            compartilhar conhecimento e tantos outros.

                        </div>
                        <div class="tab-pane fade" id="list-settings" role="tabpanel" aria-labelledby="list-settings-list">Site com intuito de vender.
                        </div>
                    </div>
                </div>
            </div>
            <div class="row mt-3    ">
                <div class="col-6 border border-light rounded p-3 text-light">
                <h5>  E-mail Profissional</h5>
                   
                     <p class=" text-center m-3">
                         <strong class="letramaisclara"> "Credibilidade para o seu endereço eletrônico".</strong> <br>
                         Eviar e-mail com o nome do seu negócio, transmite o comprometimento da empresa com a comunicação com o cliente.
                        
                     </p>
                    </div>
                <div class="col-1"></div>
                <div class="col-5 border border-light rounded p-3 text-light">
                    <h5>Criação de Logomarcas</h5>
                    <p class="m-3 text-center">Como etapa para a <strong class="letramaisclara">construção visual</strong>  da sua empresa, ter uma logo profissional é essencial para a 
                        consolidação do seu negócio. <br>
                        
                    </p>
                </div>
            </div>
        </div>
        <div class="col-1"></div>
        <div class="col-4">
            <div class="row border border-light rounded p-3 text-light">
                <h5 class="mb-4">Cartão Virtual</h5>
                <p class="text-center">Fundamental para o desenvolvimento do negócio o Cartão Virtual é a ferramenta que mantém contato com
                    o cliente. <br> <Strong class="letramaisclara">É a sua autoapresentação!</Strong> <br> Reforça a <strong class="letramaisclara">credibilidade</strong> e a
                    <strong class="letramaisclara">legitimidade</strong> do seu negócio.
                </p>
                <p class="list-group" id="list-tab" role="tablist "> <strong class="list-group-item list-group-item-action list-group-item-danger letramaisescura">Vantagens do Cartão Virtual</strong> </p>
                <li class="mt-2 letramaisclara"> Podem ir onde quer que vá!</li>
                <li class="mt-2 letramaisclara"> Facilidade de Acesso</li>
                <li class="mt-2 letramaisclara"> Indicação com apenas um click</li>
                <li class="mt-2 letramaisclara"> Divulgação em cascata</li>
                <li class="mt-2 letramaisclara"> Nunca acaba</li>
                <li class="mt-2 letramaisclara"> Facilidade para Atualizações</li>
                <li class="mt-2 letramaisclara"> Menor custo</li>
                <br>
                <br>
            


            </div>
        </div>

    </div>
    <!-- Fim Linha Serviços  -->

    <!-- Linha Sobre/Contato-->
    <div class="row">

        <!-- Coluna Sobre -->
        <div class="col text-light mt-5">
            <h4>
                A Empresa
            </h4>
            <p class="text-center">Atuamos no mercado de tecnologia na construção virtual da sua empresa, priorizando a qualidade para 
                melhor alcançar os objetivos de nossos clientes, oferecemos serviços como: desenvolvimento de sites, 
                e-mail profissional, cartão de visitas virtual e criação de logomarcas.
            </p>

        </div>
        <!-- Fim Coluna Sobre -->

        <!-- Coluna Contato -->
        <div class="col"></div>
        <!-- Fim Coluna Contato -->

    </div>
    <!-- Fim Linha Sobre/Contato -->

    <!-- Linha Reuniões On-line/Localização -->
    <div class="row">

        <!-- Coluna Reuniões On-line -->
        <div class="col">

        </div>
        <!-- Fim Coluna Reuniões On-line -->

        <!-- Coluna Localização -->
        <div class="col">

        </div>
        <!-- Fim Coluna Localização -->

    </div>
    <!-- Fim Linha Reuniões On-line/Localização -->


</div>
<!-- Fim Corpo Site -->

<!-- Footer -->
<div>
<footer class="mt-5">
    <div class="container">
        cnpj icon pasta

    </div>

</footer>

</div>
<!-- Fim Footer -->